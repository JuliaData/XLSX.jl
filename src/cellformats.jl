...
Draft implementation, built and cross-checked against:
  - JuliaData/XLSX.jl master (get_worksheet_xml_document, set_worksheet_xml_document!,
    find_child_index, get_idces, get_prefix, xml_root_element, xml_elements, xmlroot,
    getColumnWidth, getRowHeight)
  - XML v0.4.1 (Node is immutable but its .children/.attributes vectors are mutable;
    push!/pushfirst!(node, child) splice in place; Element(tag; kwargs...) constructs)
  - Real Excel-authored sheetView/pane/selection XML (SheetJS #325, SAS support thread,
    and a genuine frozenSplit sample) rather than the ECMA spec text alone.

Not yet tested against a live XLSX.jl checkout / real fixtures — next step is building
the three fixture shapes (no sheetViews, existing plain sheetView, existing frozen pane
to overwrite) and running these against them.
=#

@enum PaneKind FROZEN FROZEN_SPLIT SPLIT

#-----------------------------------------------------------------------------
# XML tree helpers (schema-order-aware insertion, prefix-safe lookup)
#-----------------------------------------------------------------------------
#
# NOTE: an earlier draft created an empty placeholder element (e.g.
# `XML.Element("sheetView")` with zero children) and then mutated it via
# `push!`/`pushfirst!`. That fails against the installed XML v0.4.1:
# `Element(tag; attrs...)` called with zero explicit children produces a node
# with `children === nothing` (a "cannot ever have children" node), and
# `push!`/`pushfirst!` refuse to touch that. Since `Node` is immutable, a
# node's `children` field can't be retrofitted from `nothing` to a vector
# after construction either. So instead: always build a node with its full,
# final set of children passed positionally at construction time, and only
# ever splice into a *parent's* children vector (which is safe once that
# parent genuinely has one) rather than into the new node itself.
#-----------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
# Selection-cell placement (confirmed against real Excel/LibreOffice XML)
#-----------------------------------------------------------------------------
...
function _apply_pane!(ws::Worksheet, kind::PaneKind, nrows::Int, ncols::Int)
    ...
    # place rather than mutate it (see note above on why we can't mutate).
    ...
    # Build the new pane/selection children (empty for a full unfreeze/removePanes).
...
!!! note "Approximate divider placement"
    ...
    since the workbook's actual default font metrics aren't read — this can
    be measurably off for workbooks using a different default font (e.g.
    current Excel defaults to "Aptos Narrow"). The divider will be close to
    the requested cell boundary but not necessarily exact; Excel/the user can
    always drag it afterward.

Requires `ws` to be writable (same requirement as [`getColumnWidth`](@ref)/
[`getRowHeight`](@ref), which this uses internally).
"""
